...
func saveToFile(data interface{ MarshalBinary() ([]byte, error) }, filename string) error {
	...
	return os.WriteFile(filename, []byte(dataBase64), 0644)
}

func readFromFile(data interface{ UnmarshalBinary([]byte) error }, filename string) error {
	dataBase64, err := os.ReadFile(filename)
	if err != nil {
		return err
	}

	dataBytes, err := base64.StdEncoding.DecodeString(string(dataBase64))
	if err != nil {
		return err
	}
...
func obliviousRiding() {
	...
	var riderSk *rlwe.SecretKey
	var riderPk *rlwe.PublicKey

	params_filename := "keys/bgv-params.b64"
	secret_key_filename := "keys/bgv-secret-key.b64"
	public_key_filename := "keys/bgv-public-key.b64"
	...
	// Try to load existing parameters and keys from files
	_, err1 := os.Stat(params_filename)
	_, err2 := os.Stat(secret_key_filename)
	_, err3 := os.Stat(public_key_filename)
	if err1 == nil && err2 == nil && err3 == nil {
		// Load existing parameters and keys
		fmt.Println("Found existing parameter and key files")
		fmt.Println()

		if err := readFromFile(&params, params_filename); err != nil {
			panic(err)
		}
		kgen := rlwe.NewKeyGenerator(params)
		riderSk, riderPk = kgen.GenKeyPairNew()
		if err := readFromFile(riderSk, secret_key_filename); err != nil {
			panic(err)
		}
		if err := readFromFile(riderPk, public_key_filename); err != nil {
			panic(err)
		}
	} else {
		// Generate new parameters and keys
		fmt.Println("Generating new parameters and keys...")
		fmt.Println()
		...
		// Save parameters, secret key, and public key
		if err := saveToFile(params, params_filename); err != nil {
			panic(err)
		}
		if err := saveToFile(riderSk, secret_key_filename); err != nil {
			panic(err)
		}
		if err := saveToFile(riderPk, public_key_filename); err != nil {
			panic(err)
		}
	}
